...
import { CalendarIcon, Loader2, Sparkles, Image as ImageIcon, Send, Clock, AlertCircle, Trash2, ArrowLeft, CheckCircle2, XCircle, FileText } from "lucide-react";
import { 
  useListPages, 
  useGetPost,
  useUpdatePost,
    ...
    updatePost.mutate(
      ...
      },
      {
        onSuccess: (updatedPost) => {
          toast({ title: "Post updated", description: "Your post changes have been saved." });
          queryClient.setQueryData(getGetPostQueryKey(id), updatedPost);
          queryClient.invalidateQueries({ queryKey: getListPostsQueryKey() });
          queryClient.invalidateQueries({ queryKey: getGetDashboardStatsQueryKey() });
        },
        onError: (error) => {
          toast({ title: "Error updating post", description: error.message || "Something went wrong.", variant: "destructive" });
        },
      }
    );
  };
    ...
    publishPost.mutate(
      { id },
      {
        onSuccess: (publishedPost) => {
          toast({ title: "Post published!", description: "Your post is now live on Facebook." });
          queryClient.setQueryData(getGetPostQueryKey(id), publishedPost);
          queryClient.invalidateQueries({ queryKey: getListPostsQueryKey() });
          queryClient.invalidateQueries({ queryKey: getGetDashboardStatsQueryKey() });
        },
        onError: (error) => {
          toast({ title: "Publishing failed", description: error.message || "Could not publish post.", variant: "destructive" });
        },
      }
    );
  };
                ...
                <CardContent className="space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-col">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <FormControl>
                              ...
                              >
                                {field.value ? format(field.value, "PPP p") : <span>Draft (Not scheduled)</span>}
                        ...
                        <FormDescription>Clear to save as a draft.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  ...
                  {!isReadOnly && (
                    ...
                    >
                      ...
                      {form.watch("scheduledAt") ? "Update Schedule" : "Save Changes"}
                    </Button>
                  )}
                </CardContent>
              </Card>
